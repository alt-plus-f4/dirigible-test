...
export class TypesRepository {
    ...
    private async triggerEvent(data: TypesEntityEvent | TypesUpdateEntityEvent) {
        const triggerExtensions = await extensions.loadExtensionModules("pet-store-settings-Types", ["trigger"]);
        ...
        producer.topic("pet-store-settings-Types").send(JSON.stringify(data));
    }
}
